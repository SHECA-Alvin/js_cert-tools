...
export default class rsaPKCS {
  static toPKCS1(key: string, msg?: NzMessageService): string|null {
    try {
      ...
      if (pkcs.result.valueBlock.value.length > 5) {
        return key;
      }

      let pkcs1Buffer = Buffer.from([0x02, 0x01, 0x00,]).buffer;

      for (let index = 1; index < pkcs.result.valueBlock.value[2].valueBlock.value[0].valueBlock.value.length; index ++) {
        pkcs1Buffer = Buffer.concat([Buffer.from(pkcs1Buffer), Buffer.from(pkcs.result.valueBlock.value[2].valueBlock.value[0].valueBlock.value[index].valueBeforeDecode),]);
      }
  ...
  static toPKCS8(key: string, msg?: NzMessageService): string|null {
    try {
